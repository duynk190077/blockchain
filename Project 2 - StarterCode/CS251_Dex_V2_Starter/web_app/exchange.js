...
const exchange_name = "masterSwap"; // TODO: fill in the name of your exchange

const token_name = "dhdCoin"; // TODO: replace with name of your token
const token_symbol = "DHD"; // TODO: replace with symbol for your token
...
const exchange_address = "0x2279B7A0a67DB372996a5FaB50D91eAA73d2eBe6";
...
async function addLiquidity(amountEth, maxSlippagePct) {
  ...
  await token_contract
    .connect(provider.getSigner(defaultAccount))
    .approve(exchange_address, Math.floor(rates.max_exchange_rate * amountEth));
  await exchange_contract
    .connect(provider.getSigner(defaultAccount))
    .addLiquidity(rates.max_exchange_rate, rates.min_exchange_rate, {
      value: ethers.utils.parseUnits(String(amountEth), "wei"),
    });
}
...
async function removeLiquidity(amountEth, maxSlippagePct) {
  ...
  await exchange_contract
    ...
    .removeLiquidity(
      amountEth,
      rates.max_exchange_rate,
      rates.min_exchange_rate
    );
}
...
async function removeAllLiquidity(maxSlippagePct) {
  ...
  await exchange_contract
    .connect(provider.getSigner(defaultAccount))
    .removeAllLiquidity(rates.max_exchange_rate, rates.min_exchange_rate);
}
...
async function swapTokensForETH(amountToken, maxSlippagePct) {
  ...
  await exchange_contract
    .connect(provider.getSigner(defaultAccount))
    .swapTokensForETH(amountToken, rates.max_exchange_rate);
}
...
async function swapETHForTokens(amountEth, maxSlippagePct) {
  ...
  await exchange_contract
    .connect(provider.getSigner(defaultAccount))
    .swapETHForTokens(rates.max_exchange_rate, {
      value: ethers.utils.parseUnits(String(amountEth), "wei"),
    });
}
